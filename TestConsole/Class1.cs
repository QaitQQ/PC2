...
namespace InstagramClient
{
    public class Instagram : IDisposable
    {
        ...
        {
            using (Instagram instagram = new Instagram())
            {
        ...
        {
            string token = await GetAccessToken(config.ClientId, config.RedirectUri, BuildScopeForUri(scopes));
            ...
            Users users = new Users(config, auth);
            UserResponse self = await users.GetSelf();
            auth.User = self.Data;
        ...
        private static string BuildScopeForUri(List<OAuth.Scope> scopes)
        {
            ...
            foreach (OAuth.Scope s in scopes)
            {
                if (scope.Length > 0)
                {
                    scope.Append("+");
        ...
        public async Task<string> GetAccessToken(string clientId, string redirectUri, string scope)
        {
            ...
            using (HttpClientHandler temporaryHandler = new HttpClientHandler())
            {
                temporaryHandler.AllowAutoRedirect = false;
                temporaryHandler.CookieContainer = m_Handler.CookieContainer;

                using (HttpClient temporaryClient = new HttpClient(temporaryHandler))
                {
                    temporaryClient.BaseAddress = m_Client.BaseAddress;

                    using (HttpResponseMessage response = await temporaryClient.SendAsync(request))
                    {
            ...
            int position = tokenFragment.IndexOf('=');
        ...
        public async Task<bool> LoginAsync(string username, string password)
        {
            ...
            // готовим запрос
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, " https://instagram.com/accounts/login/ajax/")
            {
            ...
            using (HttpResponseMessage response = await m_Client.SendAsync(request))
            {
                LoginInfo info = JsonConvert.DeserializeObject<LoginInfo>(await response.Content.ReadAsStringAsync());
                return info.authenticated;
            }
        }

        /// <summary>
        /// Получает из Cookies CSRF-токен
        /// </summary>
        /// <returns>CSRF-токен</returns>
        private string GetCSRFToken()
        {
            CookieCollection cookies = m_Handler.CookieContainer.GetCookies(m_Client.BaseAddress);
