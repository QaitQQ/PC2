...
namespace SiteApi.IntegrationSiteApi.APIMarket.Ozon.Post
{
    public class OzonPostAct : OzonPost
    {
        ...
        {
            HttpWebRequest httpWebRequest = GetRequest(@"v2/posting/fbs/act/create");
            ...
            DateTime parsedDate = DateTime.Parse(orders[0].ShipmentDate!);
            string formData = $"{parsedDate:O}";
            ...
            try
            {
                ...
                using (StreamWriter streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                {
                    string json = JsonConvert.SerializeObject(XR); streamWriter.Write(json);
                }
                httpResponse = (HttpWebResponse)httpWebRequest.GetResponse();
            }
            catch (Exception e)
