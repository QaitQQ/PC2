            ...
            if x.DeviceData != None:
                ...
                if x.device.deviceType.name == "valve":
                    try:
                        ...
                        pass
                if  x.device.deviceType.name == "relay":
                    try:
                        mess = json.loads(cd, object_hook=lambda d: SimpleNamespace(**d))
...
def NewDev(dev_id, tape, requestData):
    ...
    if not Device.objects.filter(sn=dev_id).exists():          
        ...
        devStatus =  DeviceStatus.objects.create(device = device, data =datetime.now(tzinfo))
...
def data_ex(request):
        ...
        if device != None:
            ...
            if not DeviceData.objects.filter(device = device).exists():
                ...
                if DateOfLastOperation != None and DateOfLastOperation!= "":                  
                    ...
                    data_obj = json.loads(cd, object_hook=lambda d: SimpleNamespace(**d))   
                    data_obj.DateOfLastOperation = DateOfLastOperation
                    pd = json.dumps(data_obj, ensure_ascii=False, default=get_obj_dict,)
                    pd = pd.replace("\"","'")
                    dev_dat.data = pd
                    dev_dat.save()
            ...
            if create == "new":
                ...
                return JsonResponse ({"Result":"NoDevice"},status=status.HTTP_400_BAD_REQUEST  )
    ...
    serData =  SingleDataSerializer(DeviceData.objects.filter(device = device), many=True).data      
    return JsonResponse({"Result":serData, "Time":datetime.now(tzinfo).strftime('%H:%M:%Y_%d:%m')},status=status.HTTP_201_CREATED, safe=False) 
...
def add_opt(request):
    ...
    if mess["hour"]!="" and mess["minutes"] !="" and mess["delay"]!="":
        ...
        pd = json.dumps(data, ensure_ascii=False, default=get_obj_dict,)
